...
# install.packages(c("panelView","bacondecomp","did2s","did","DIDmultiplegt"))
...
## ----------------------------------------------------------------------------------------------------------------------------------
## Exclude New Hampshire, Pennsylvania, Vermont, Washington:
Vax_adj <- Vax_weekly %>% 
  dplyr::filter(!(State %in% c("NH","PA","VT","WA")))
...
agg.cal <- aggte(attgt, type="calendar")
summary(agg.cal)
