...
4.  `DIDmultiplegt` implements the first-difference approach of [de Chaisemartin and d'Haultfoeuille (2020)](https://doi.org/10.1257/aer.20181169) (if you're on a Mac, you may need to install [Xcode](https://developer.apple.com/xcode/) from the App Store and [XQuartz](https://www.xquartz.org) first and re-start R). Versions of this approach that can handle a wider array of settings, laid out in [de Chaisemartin and d'Haultfoeuille (2023)](https://doi.org/10.1093/ectj/utac017) and [de Chaisemartin and d'Haultfoeiulle (2024)](https://doi.org/10.1162/rest_a_01414), along with sped-up analytic standard errors are implemented in the `DIDmultiplegtDYN`, which requires Java for installation.

We now load the required libraries (except `DIDmultiplegt`, which we will load later).
...
Some of these visualizations can be accomplished with the `panelView` package instead; see <https://yiqingxu.org/packages/panelview/index.html> for details.
...
We re-plot this adjusted group of states and weeks.
...
This is the basic dynamic estimator that arises from including leads/lags (except -1, the last pre-treatment period) in the TWFE model. There is some evidence for non-parallel trends: the lead times consistently have positive point estimates, indicating the treated states were growing more slowly than untreated states prior to treatment. These are decreasing, indicating that this effect was disappearing, or possibly changing. Under heterogeneous treatment effects (either calendar time-varying treatment effects or different dynamic effects by adoption cohorts), there can still be cross-lag contamination and negative weights. This even means that apparent pre-trends may not be reliable (i.e., non-zero lead estimates can result from heterogeneous dynamic effects rather than a violation of parallel trends).
...
In all cases, note how the standard errors tend to increase further from 0, as there are fewer treated (and in some cases, control) units available to make those estimates.

Aggregating to a single parameter depends on the desired interpretation and variance properties.
...
-   The "simple" aggregation weights all group-time effects proportional to group size;

-   The "dynamic" aggregation combines to the time-on-treatment level and reports each one ("event study"-style);

-   The "group" aggregation combines to the timing group level and reports each one; and

-   The "calendar" aggregation combines to the time period level and reports each one.
...
We can plot the event-study plot again from the "dynamic" aggregation results using the `ggdid` function:
...
While there are differences among the approaches, nearly all of the methods used here point to a small and highly uncertain effect. In addition, all leads suggest some violation of parallel trends. Adjusting for the previous week's cumulative first dose percentage leads to more zero leads and a consistently positive post-treatment estimate, which may indicate that covariate adjustment and/or using new doses as the outcome and a log scale might help solve some of the assumption issues.

Overall, the differences among estimators are small relative to concerns about the validity of the analysis overall. However, they do point out potential issues about heterogeneity of effects based on state and time.

## Discussion Questions

-   Are this question and this data set a good fit for a DID/staggered adoption analysis? Why or why not?

-   What could explain the heterogeneities that are observed?
