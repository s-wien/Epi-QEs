...
ggplot(WAE, mapping=aes(x=Time, y=Y, group=Unit, color=Unit,
                        shape=X)) + 
  geom_point(size=4) + geom_line(lwd=1.5) + theme_bw() +
...
[Sun and Abraham (2021)](https://doi.org/10.1016/j.jeconom.2020.09.006) use the approach only comparing to a clean "control" $C$ that is either never-treated or last-treated. Their regression approach then implicitly weights by population share in each timing group.
